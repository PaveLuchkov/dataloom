...
export default function PandasExportModal({ code, onClose }) {
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url;
    a.download = `lineage-${new Date().toISOString().slice(0, 10)}.py`;
    a.click();
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700">
          <span className="text-sm font-semibold text-slate-200">Export pandas</span>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200 text-lg leading-none">×</button>
        </div>

        <div className="px-4 pt-2 pb-1 text-[11px] text-slate-400">
          Best-effort scaffold from the lineage graph — review before running. A node's own code snippet overrides its generated line.
        </div>

        <div className="px-4 pt-1 pb-1 flex-1 overflow-y-auto min-h-0">
          <pre
            className="w-full rounded-lg text-xs font-mono p-3 overflow-auto"
            style={{ background: '#0f172a', border: '1px solid #334155', color: '#e2e8f0', lineHeight: 1.7, maxHeight: '55vh', whiteSpace: 'pre' }}
          >
            {code}
          </pre>
        </div>
